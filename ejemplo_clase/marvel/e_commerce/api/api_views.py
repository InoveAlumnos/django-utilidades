...
from e_commerce.tasks import send_email_auth_task
    ...
    def post(self, request):
        ...
        if user_login_serializer.is_valid():
            ...
            if _account:
                ...
                if _created:
                    # Envío un email de manera asincrónica con datos del usuario.
                    # Para ejecutar una task de forma asincrónica podemos usar
                    # ".delay()" o "apply_async()".
                    _subject = '[Generación de Token]'
                    _message = (
                        f'USERNAME: "{_account.username}"\n'
                        f'TOKEN: "{_token}".'
                    )
                    send_email_auth_task.delay(
                        subject=_subject,
                        message=_message,
                        receiver=_account.email
                    )
                return Response(
